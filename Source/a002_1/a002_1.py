...
learn the contents of an csv file:
    ...
    a002_1.py --learn --input input.csv --output model.json

estimate 1 task:

    a002_1.py --estimate --text "hello world" --model model.json

estimate a csv file full of tasks (for algorithm validation purposes):

    a002_1.py --validation --input input.csv --output output.csv --model model.json
...
if args.learn:
    """learn from csv"""
    print ("Algorithm is learning from presented data... one moment please")
    df = las.load_csv(args.input)
    
    mean = df.DurationInSeconds.mean()
    las.save_json(args.output, dict(mean = mean))

    print ("learned that the mean is:", mean) 
